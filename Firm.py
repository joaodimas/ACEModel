...
class Firm :
    ...
    def selectFirmToImitate(self):
        ...
        Logger.trace("[FIRM {:d}] Point in CDF: {:.3f}", (self.firmId, selection), industry=self.industry)
        ...
        for firm in otherFirms:
            ...
            if(cdf < selection <= cdf + probOfBeingObserved):
                Logger.trace("[FIRM {:d}] Competitor {:d} was observed. Point in CDF: {:.3f}", (self.firmId, firm.firmId, selection), industry=self.industry)
                return firm
